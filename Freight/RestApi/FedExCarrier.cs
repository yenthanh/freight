    ...
    public class FedExCarrier : BaseCarrier
    {
        ...
        public override List<PriceResultItem> GetExportPrice(string countryTo, string deliverType, string serviceType, string packageType, float weight, string region)
        {
            ...
            foreach (var s in lstSheets)
            {
                ...
                string zoneName = GetZoneNumberByServiceType(tblZone, countryTo, anotherCountryName, deliverType, s.SERVICE_ID, region);
        ...
        /// <returns></returns>
        private string GetZoneNumberByServiceType(DataTable tbl, string countryTo,string anotherName, string deliverType, string serviceType, string region)
        {
            ...
            {
                rows = this.TryToGetZoneByAnotherName(tbl, this.COUNTRY_HEADER_COL, anotherName);
                if (rows == null) return "";
            }
            if (rows.Length == 1)
            {
